...
### Business Plan → Slide Plan → PPT 렌더링 (빈 슬라이드 방지)

1. **`build_business_plan(inputs, agent_outputs)`** (`src/autopm/ppt/content_builder.py`): 인터뷰 필드·Crew Markdown·구조화 출력을 **단일 `business_plan` dict**로 통합합니다. 필드가 비면 `ensure_nonempty` 및 **포스코/원가 등 키워드 기반 풍부한 fallback**으로 채웁니다.
2. **`build_slide_deck_spec(business_plan, options)`** (`src/autopm/ppt/slide_builder.py`): `business_plan`을 **최소 10~11장** `SlideDeckSpec` JSON으로 매핑합니다. 각 슬라이드에 `title`, `objective`, `key_message`, `visual_type`, `content`(bullets / rows / cards / steps / risks 등)를 **강제로 채웁니다**.
3. **`validate_slide_deck_content` + `ensure_valid_deck`**: 생성 직전 검증(슬라이드 수, key_message, WBS·예산·리스크 행 수, AS-IS·TO-BE steps 등) 후, 실패 시 **`business_plan`으로 덱 재생성** → 그래도 부족하면 **hardcoded 최소 덱**으로 복구합니다.
4. **Composer** (`ppt_composer.py`, `layout_engine.py`, `visual_builder.py`): `visual_type`별로 **표·프로세스 흐름·카드·KPI 스트립**을 python-pptx로 그립니다. LLM `slide_plan`이 얇아도 **`business_plan` 기반 덱이 주 소스**이므로 제목만 있는 장이 나오지 않도록 합니다.
5. **디버그 출력**: `outputs/business_plan.json`, `outputs/slide_plan.json`, `outputs/content_coverage_report.json` — Streamlit **Business Plan JSON**, **Content Coverage** 탭에서 확인 가능합니다.
...
| `business_plan.json` | Agent·fallback 통합 구조 (slide·PPT의 단일 소스) |
| `slide_plan.json` | 슬라이드 스펙 |
| `content_coverage_report.json` | 슬라이드 본문·표·카드·WBS/예산/리스크 검증 요약 |
...
- **데모만(API 키 없이): [DEPLOY_STREAMLIT_DEMO.md](./DEPLOY_STREAMLIT_DEMO.md)** — Streamlit Cloud에서 클릭 순서만 따르면 됩니다.
- 전체 옵션: **[PUBLIC_URL_DEPLOY.md](./PUBLIC_URL_DEPLOY.md)** (Render Docker 등).
- [Streamlit Community Cloud](https://streamlit.io/cloud): Main file **`app.py`**, 의존성은 **`requirements.txt`**.
...
- **Business plan 레벨**: API/에이전트 출력이 짧거나 비어도 `build_business_plan`이 Markdown 섹션 파싱·기본 문장·**시나리오별 긴 샘플(예: Mini ERP 원가)** 로 필드를 채워 `business_plan.json`이 항상 의미 있는 볼륨을 갖도록 합니다.
- **Slide 레벨**: `build_slide_deck_spec`이 각 `visual_type`에 맞는 `content` 키를 채우고, 검증 실패 시 동일 소스로 덱을 재생성합니다.
- LLM/API 실패: Markdown + **검증된** 10장+ 슬라이드 덱 + `graphics_spec` 결정론 보강
- matplotlib 실패: Pillow PNG 또는 python-pptx 도형만 사용
- 그래픽 단계 전체 실패: `apply_visual` / `layout_engine` 기존 경로

### Content Coverage Report

- PPT 저장 직전 `validate_slide_deck_content`가 **총 슬라이드 수**, **본문이 채워진 슬라이드 수**, **빈 content 슬라이드 수**, **표/카드형 슬라이드 수**, **WBS·예산·리스크 데이터 존재**, **AS-IS·TO-BE steps 동시 충족 여부**를 집계해 `content_coverage_report.json`에 기록합니다. Streamlit **Content Coverage** 탭에서 동일 지표를 확인할 수 있습니다.
