...
User Input (제목·인터뷰)
  ...
  → SlideDeckSpec JSON → **Presenton API** (권장) 또는 python-pptx (폴백)
...
| `AUTOPM_PPT_API` | `presenton`(기본) / `python-pptx` / `both` |
| `PRESENTON_BASE_URL` | Presenton 서버 (Docker 기본 `http://127.0.0.1:5000`) |
| `PRESENTON_USERNAME` / `PRESENTON_PASSWORD` | self-hosted HTTP Basic |
| `PRESENTON_API_KEY` | 클라우드 Bearer (`sk-presenton-...`) |

### Presenton으로 고품질 PPT (권장)

[Presenton](https://github.com/presenton/presenton)을 Docker로 띄운 뒤 `.env`에 인증을 넣으면, **PPT Composer Agent 최종 JSON**(`slides_markdown` + `content_generation=preserve`)이 Presenton API로 `outputs/project_plan.pptx`를 만듭니다. 서버가 없거나 실패하면 **python-pptx**로 자동 폴백합니다.

```powershell
# 권장: compose + MCP URL 자동
.\scripts\start_presenton.ps1
# 또는
docker compose -f docker-compose.presenton.yml up -d
```

```env
AUTOPM_PPT_API=presenton
AUTOPM_PRESENTON_USE_MCP=true
PRESENTON_BASE_URL=http://127.0.0.1:5000
PRESENTON_MCP_URL=http://127.0.0.1:5000/mcp
PRESENTON_USERNAME=admin
PRESENTON_PASSWORD=changeme123
```

PPT 생성 시 **Presenton MCP** (`generate_presentation`) → 실패 시 REST API → 실패 시 python-pptx 순으로 폴백합니다.  
Cursor IDE MCP 예시: `cursor-mcp.example.json` → `.cursor/mcp.json`에 복사 후 인증 수정.
...
- **PPT**: Presenton 미설정·실패 시 `python-pptx` + `build_fallback_slide_deck` 결정론 렌더링
